...
defineRoute("/onboarding",page("Set up your profile",async()=>onboardingPage((await identityRepository.getCurrentUser())?.personalDetails)));
...
function continueAfterIdentity(){const destination=sessionStorage.getItem("mygita.pending.destination")||"/discover";sessionStorage.removeItem("mygita.pending.destination");navigate(destination);}

document.addEventListener("click",async event=>{const target=event.target;if(!(target instanceof Element))return;const action=target.closest("[data-action]");if(!(action instanceof HTMLElement))return;const name=action.dataset.action;if(name==="toggle-menu"||name==="toggle-mobile-menu"){const menu=document.getElementById(name==="toggle-menu"?"account-menu":"mobile-menu");if(!menu)return;const opening=menu.hidden;menu.hidden=!opening;action.setAttribute("aria-expanded",String(opening));}if(name==="toggle-password"){const input=document.getElementById(action.dataset.target||"");if(!(input instanceof HTMLInputElement))return;const showing=input.type==="text";input.type=showing?"password":"text";action.textContent=showing?"Show":"Hide";action.setAttribute("aria-label",`${showing?"Show":"Hide"} password`);action.setAttribute("aria-pressed",String(!showing));}if(name==="skip-profile")continueAfterIdentity();if(name==="sign-out"){await identityRepository.signOut();navigate("/discover");}if(name==="reset-fixtures"){await Promise.all([identityRepository.reset(),journeyRepository.reset()]);navigate("/review");}});
