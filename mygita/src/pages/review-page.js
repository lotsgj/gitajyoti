...
export function reviewPage(){
  ...
  const groups=[
    ...
    ["Journey & profile",[["My Journey","#/journey"],["Activity detail","#/activity/activity-sara-teaching"],["Activity session","#/activity/activity-sara-teaching/session"],["My profile","#/profile"]]],
    ["System states",[["Loading","#/states/loading"],["Empty","#/states/empty"],["Error","#/states/error"],["Offline","#/states/offline"],["Not found","#/unknown"]]],
  ];
  return `<section class="review-page"><p class="eyebrow">Development tools</p><h1>Screen map</h1><p>Open every distinct MyGita screen from one place using the selected data provider.</p><div class="review-grid">${groups.map(([title,links])=>`<section class="panel"><h2>${title}</h2><ul>${links.map(([label,href])=>`<li><a href="${href}">${label}</a></li>`).join("")}</ul></section>`).join("")}</div>${button({label:"Reset development data",variant:"secondary",type:"button",attributes:'data-action="reset-fixtures"'})}</section>`;
}
