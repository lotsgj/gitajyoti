...
export function profileEditor(profile={}, {afterRegistration=false}={}) {
  ...
  const actions=afterRegistration
    ? `<div class="form-actions">${button({label:"Save profile",type:"submit"})}${button({label:"Skip for now",variant:"secondary",type:"button",attributes:'data-action="skip-profile"'})}</div>`
